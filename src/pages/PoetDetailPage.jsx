...
const PoetDetailPage = () => {
  ...
  const parseLifeStory = (text) => {
    ...
    lines.forEach((line, index) => {
      ...
      // Форматы: "YYYY — событие", "YYYY год — событие", "YYYY-YYYY — событие", "YYYY: событие",
      // "Осень YYYY — событие", "Январь YYYY: событие"
      const isHeader = /^(Весна|Лето|Осень|Зима|Январь|Февраль|Март|Апрель|Май|Июнь|Июль|Август|Сентябрь|Октябрь|Ноябрь|Декабрь|Начало|Конец|Середина|Наши)?\s*\d{4}(\s*г(?:\.|од(?:а)?)?)?(\s*[-–—]\s*\d{4})?\s*([-–—:]|\.\s)/.test(trimmedLine);
