...
const AdminPage = () => {
  ...
  const getContenders = () => {
    ...
    const isOverallMode = assignUser === '__overall__';

    if (assignCategory === 'overall') {
      // Общий балл — либо средний по всем (overall mode), либо персональный score
      const scoresMap = poets.map(poet => ({
        ...poet,
        score: isOverallMode ? calculateAverageScore(poet.id) : calculateScore(assignUser, poet.id)
      })).filter(p => p.score > 0);
      ...
      // Категория — либо среднее по всем оценкам, либо персональная оценка
      if (isOverallMode) {
        const rankings = getCategoryRankings(assignCategory);
        if (rankings.length === 0) return [];
        const top = rankings[0].averageRating;
        return rankings
          .filter(r => Math.abs(r.averageRating - top) < 0.01)
          .map(r => ({ ...r.poet, rating: r.averageRating }));
      }
  ...
  // Назначить победителя — персонально или в общем рейтинге
  const assignLeader = async (poetId) => {
    try {
      if (assignUser === '__overall__') {
        // Ручное назначение общего победителя категории (заменяет дуэль)
        const path = `overallDuelWinners/${assignCategory}`;
        await set(ref(database, path), { winner: poetId, manual: true });
      } else {
...
<h2 className="section-title">Награды</h2>
          <p className="section-hint">Назначить личного лидера (Максим/Олег/Люба) или общего победителя категории, если средние баллы совпали</p>
            ...
            <div className="assign-row">
              <label>
                ...
                <select value={assignUser} onChange={(e) => setAssignUser(e.target.value)}>
                  ...
                  <option value="lyuba">Люба</option>
                  <option value="__overall__">Общий</option>
                </select>
              </label>
          ...
          <div className="modal-content assign-leader-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="modal-title">
              Назначить победителя: {
                assignUser === 'maxim' ? 'Максим'
                  : assignUser === 'oleg' ? 'Олег'
                  : assignUser === 'lyuba' ? 'Люба'
                  : 'Общий рейтинг'
              }
            </h2>
