...
- Всё должно выглядеть как последовательная, живая история человека от детства до смерти (если человек еще жив, то до настоящего времени).
...
- это могут быть как факты поднимаюшие мораль (условно хорошие) так и факты понижаюшие мораль (условно плохие)
- Никаких оценок, комментариев, выводов
- Если факт ничего не говорит о морали, принципах, то не добавляй.
- НЕ дублируй факты из блока биографии выше.
- Верни только новые упущенные факты или более глубокие детали, которых нет в биографии.
- Если новых/уточняющих фактов нет, верни пустую строку.
 ...
 * @returns {string} - Промпт для AI
 */
export const generatePoetDramaPrompt = (poetName, biography = '') => {
  const biographyBlock = biography?.trim()
    ? `УЖЕ ИЗВЕСТНАЯ БИОГРАФИЯ (НЕ ПОВТОРЯЙ ЭТИ ФАКТЫ):\n${biography.trim()}\n`
    : 'УЖЕ ИЗВЕСТНАЯ БИОГРАФИЯ: отсутствует.\n';
...
Сначала предоставь Внешние черты, рост (только если точно известно) и другие параметры
...
Требования:
1. Это должен быть реальный поэт (любой эпохи от классицизма до современности)
2. Он должен писать на русском языке
3. Выбери СЛУЧАЙНОГО поэта
4. ${excludeList}
