...
/**
 * Промпт для генерации информации о влиянии поэта
 * @param {string} poetName - Имя поэта
 * @returns {string} - Промпт для AI
 */
export const generatePoetInfluencePrompt = (poetName) => {
  return `Дай краткую и по делу характеристику влияния поэта ${poetName}: масштаб культурного наследия, реальные исторические или общественные воздействия - если такого влияния нет, то так и говори. а также значимые личные поступки, раскрывающие его как человека (хорошего или плохого). Указывай только действительно важные и уникальные факты, без общих слов, без биографии.
ВАЖНО - не нужно давать оценок - только факты и конкретные примеры, без интерпретаций. Не используй смайлики в тексте. Коротко и понятно.
ФОРМАТ - СЛЕДУЙ СТРОГО ЕМУ - никакого лишнего текста даже общего заголовка не нужно
пункты с описанием - каждый пункт с новой строки начинай со слова ПУНКТ
Суперкотортко и самое важное`;
};
...
export const generatePoetCreativityPrompt = (poetName) => {
  return `Дай короткую сводку информации о творчестве ${poetName}, мне нужно очень-очень коротко и по делу и понятно.

Следуй формату ниже. ВАЖНО: от себя не добавляй никакого текста, заголовков, смайликов. ВАЖНО: Каждый пункт с новой строки!

Основная тематика: текст
...
export const generateAIRatingPrompt = (poetName, existingRatings = []) => {
  ...
  if (existingRatings.length > 0) {
    const ratingsText = existingRatings
      .slice(0, 10) // Берем максимум 10 поэтов для контекста
      .map(p => 
        `${p.name}: Творчество ${p.ratings.creativity}, Влияние ${p.ratings.influence}, Драма ${p.ratings.drama}, Красота ${p.ratings.beauty}`
      )
      .join('\n');
...
1. Творчество - Здесь оценивается литературный талант поэта: образность, метафоричность, богатство языка, оригинальность стиля, ритм, мелодичность, поэтика, глубина мыслей, эмоциональная насыщенность - как будто ты оцениваешь свое впечатление от стихов (именно стихи а не любые произведения)

2. Влияние - + личные поступки
- масштаб культурного наследия
- реальные исторические или общественные воздействия
- значимые личные поступки, раскрывающие его как человека (хорошего или плохого), плохие/эгоистичные естественно снижают балл
...
- ухоженность, внешний вид${contextSection}
...
export const parseAIRating = (aiResponse) => {
  ...
  try {
    ...
    const influenceMatch = aiResponse.match(/Влияние[:\s]+([0-5](?:\.[05])?)/i);
...
export const generateAITiebreakerPrompt = (poets) => {
  const poetsList = poets.map((poet, index) => 
    `${index + 1}. ${poet.name} (Творчество: ${poet.aiRatings.creativity}, Влияние: ${poet.aiRatings.influence}, Драма: ${poet.aiRatings.drama}, Красота: ${poet.aiRatings.beauty})`
  ).join('\n');
  
  return `Ты — искусственный интеллект и критик, который оценивает поэтов.

У нескольких поэтов получился одинаковый средний балл по всем категориям (Творчество, Влияние, Драма, Красота).
